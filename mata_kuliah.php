...
?>
        ...
        <a href="index.php?page=matkul&aksi=create" class="btn btn-primary">Tambah Data</a>
        <table class="table" id="dataTable">
            ...
            <tbody>
                <?php
                $query = mysqli_query($db, "SELECT mata_kuliah.*, dosen.nama_dosen FROM mata_kuliah LEFT JOIN dosen ON mata_kuliah.dosen_nip = dosen.nip");
                $nomor = 1;
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $data['semester'] ?></td>
                        <td>
                            <a href="index.php?page=matkul&aksi=update&kode=<?= $data['kode_mk'] ?>" class="btn btn-warning">Edit</a>
                            <?php if ($_SESSION['level'] == 'admin') { ?>
                                <a href="proses_matakuliah.php?proses=hapus&kode=<?= $data['kode_mk'] ?>" onclick="return confirm('Apakah anda yakin menghapus data ini ?')" class="btn btn-danger">Hapus</a>
                            <?php } ?>
                        </td>
                    </tr>
                <?php } ?>
