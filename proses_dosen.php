...
if (isset($_GET['proses'])) {
    ...
    if ($proses == 'simpan') {
        if (isset($_POST['submit'])) {
            ...
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
                ...
                if (move_uploaded_file($foto_tmp, $foto_folder)) {
                    $query = mysqli_query($db, "INSERT INTO dosen (nip, nama_dosen, prodi_id, foto)  VALUES ('$nip', '$nama_dosen', '$prodi_id', '$foto_folder')");
        ...
        if (isset($_POST['submit'])) {
            ...
            // Check NIP if changed
            if ($old_nip != $nip) {
                $cekNIP = mysqli_query($db, "SELECT * FROM dosen WHERE nip = '$nip'");
                if (mysqli_num_rows($cekNIP) > 0) {
                    echo "<script>alert('Data gagal diupdate! NIP sudah terdaftar'); window.location='index.php?page=dosen&aksi=update&id=$old_nip'</script>";
                    exit();
                }
            }
            ...
            if (!empty($foto_name)) {
                ...
                if (move_uploaded_file($foto_tmp, $foto_folder)) {
                    ...
                    echo "<script>alert('Gagal mengunggah foto'); window.location='index.php?page=dosen&aksi=update&id=$old_nip'</script>";
                    exit();
                }
            } else {
                $foto_folder = $_POST['existing_foto'];
            }
