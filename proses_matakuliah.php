...
if(isset($_GET['proses'])) {
    ...
    if($proses == 'simpan') {
        if(isset($_POST['submit'])) {
            ...
            if($query) {
                ...
                echo "<script>alert('Data Gagal Disimpan'); window.location='index.php?page=matkul&aksi=create'</script>";
            }
        }
    }
    elseif($proses == 'edit') {
        if(isset($_POST['submit'])) {
            ...
            $query = mysqli_query($db, "UPDATE mata_kuliah SET nama_mk='$nama_mk', sks='$sks', dosen_nip='$dosen_nip', semester='$semester' WHERE kode_mk='$kode_mk'");
            if($query) {
    ...
    elseif($proses == 'hapus') {
        ...
        if($_SESSION['level'] != 'admin') {
            echo "<script>alert('Anda tidak memiliki izin untuk menghapus data.'); window.location='index.php?page=matkul'</script>";
            exit;
        }
